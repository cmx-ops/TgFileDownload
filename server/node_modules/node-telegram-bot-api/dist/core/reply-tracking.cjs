...
exports.REPLY_NAMESPACE = void 0;
exports.expectReply = expectReply;
exports.matchReply = matchReply;
exports.forgetReply = forgetReply;
exports.expectCallback = expectCallback;
exports.matchCallback = matchCallback;
exports.forgetCallback = forgetCallback;
exports.taggedReplies = taggedReplies;
/** The `ext` namespace this layer stores its tables under. */
exports.REPLY_NAMESPACE = "reply";
...
function touch(ctx) {
    ...
    const state = handle.ext(exports.REPLY_NAMESPACE, () => ({ replies: {}, presses: {} }), (slot) => isTable(slot.replies) && isTable(slot.presses));
...
function record(table, messageId, marker, options, config, now) {
...
function used(entry, config, now) {
    if (config === undefined) {
        return;
    }
...
function evict(state, config) {
    ...
    while (maxEntriesPerChat !== undefined &&
        i < victims.length &&
        victims.length - i > maxEntriesPerChat) {
        dropNext();
    }
    while (maxBytesPerChat !== undefined &&
        i < victims.length &&
 ...
 */
function expectReply(ctx, messageId, marker = {}, options) {
...
function forgetReply(ctx, messageId) {
 ...
 */
function expectCallback(ctx, messageId, marker = {}, options) {
    const { state, config, now } = touch(ctx);
 ...
 */
function matchCallback(ctx, options) {
...
function forgetCallback(ctx, messageId) {
