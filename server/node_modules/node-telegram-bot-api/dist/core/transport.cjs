...
class Transport {
    ...
    async request(method, params, signal) {
        ...
        for (let attempt = 0; attempt <= ctx.maxRetries; attempt++) {
            const outcome = await this.attempt(ctx, attempt);
            if (!outcome.retry) {
                return outcome.result;
            }
            await (0, delay_js_1.delay)(outcome.waitMs, signal);
        }
    ...
    async attempt(ctx, attempt) {
        ...
        try {
            ...
            return this.onTransportError(err, ctx, attempt);
        }
        finally {
            cleanup();
        }
        return this.classify(ctx, attempt, response, text);
    }
    /** Transient throw (fetch reject / body-read failure / our timeout): retry or surface. */
    onTransportError(err, ctx, attempt) {
        ...
        if ((0, errors_js_1.isAbortError)(err)) {
            throw new errors_js_1.TimeoutError(`Request timed out: ${ctx.method}`, { cause: err });
        }
        throw new errors_js_1.NetworkError(`Network request failed: ${ctx.method}`, { cause: err });
    }
    /** Map a completed HTTP response to a result, a bounded retry, or a thrown error. */
    classify(ctx, attempt, response, text) {
        // Server-side 5xx is transient: retry without parsing the body.
        if (response.status >= 500) {
            return this.onServerError(ctx, attempt, response, text);
        }
        const json = parseJson(text, ctx.method);
        if (json.ok) {
            log("<- %s ok", ctx.method);
    ...
    onServerError(ctx, attempt, response, text) {
        if (attempt < ctx.maxRetries) {
            const wait = (0, delay_js_1.backoff)(attempt + 1, this.retryBackoffMs, MAX_BACKOFF);
            log("%s HTTP %d; retry %d/%d in %dms", ctx.method, response.status, attempt + 1, ctx.maxRetries, wait);
            return { retry: true, waitMs: wait };
        }
        ...
        throw new errors_js_1.NetworkError(`Server error ${response.status} on ${ctx.method}`);
    }
    /** Handle an `{ ok: false }` envelope: honor a capped 429 retry_after, else throw. */
    onApiError(ctx, attempt, json) {
        if (json.error_code === errors_js_1.HTTP_STATUS_TOO_MANY_REQUESTS && attempt < ctx.maxRetries) {
            ...
            log("%s 429; retry_after %ds exceeds maxRetryAfterMs (%dms) - surfacing", ctx.method, retryAfter, this.maxRetryAfterMs);
        }
...
function buildInit(body, headers, signal) {
    const init = {
        method: "POST",
        body,
        headers,
