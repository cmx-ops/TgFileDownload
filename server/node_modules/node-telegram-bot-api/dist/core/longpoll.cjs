...
function planRetry({ err, conflicts, retry, retryDelayMs, conflictRetryDelayMs, maxConflictRetries, onError, }) {
    const pollConflict = (0, errors_js_1.isPollConflict)(err);
    if (!retry || !((0, errors_js_1.isTransientError)(err) || pollConflict)) {
        throw err;
    }
...
async function recover(ctx) {
    ...
    try {
        await (0, delay_js_1.delay)(plan.wait, signal);
    }
    catch {
