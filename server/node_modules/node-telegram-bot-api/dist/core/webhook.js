 ...
 */
function rejectBadMethod(request) {
 ...
 */
function rejectBadSecret(request, secretToken) {
 ...
 */
function dispatchInBackground(bot, update, waitUntil) {
 ...
 */
async function dispatchAwaiting(bot, update) {
    try {
...
export function webhookCallback(bot, options = {}) {
    ...
    if (secretToken === undefined) {
        if (allowUnauthenticated !== true) {
            throw new TelegramBotError("webhookCallback requires `secretToken` (matching setWebhook's secret_token). " +
        ...
        throw new TelegramBotError("Invalid `secretToken`: must be 1-256 characters of A-Z, a-z, 0-9, _ or - (per setWebhook).", { code: "EPARAM" });
    }
