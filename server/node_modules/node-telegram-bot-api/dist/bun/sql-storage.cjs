...
exports.SqlSessionStorage = void 0;
...
class SqlSessionStorage {
    ...
    constructor(options = {}) {
        ...
        this.sql = options.sql ?? (options.url !== undefined ? new bun_1.SQL(options.url) : bun_1.sql);
    }
    /**
    ...
    async read(key) {
        await this.init();
        const [row] = (await this.sql `SELECT value FROM ${this.ref} WHERE key = ${key}`);
        if (row === undefined) {
            return undefined;
        }
...
exports.SqlSessionStorage = SqlSessionStorage;
//# sourceMappingURL=sql-storage.cjs.map